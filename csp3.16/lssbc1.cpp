...
bool LSSBC1(domain* dom,variable* x,constraint* C,int account,int** matrix,int*** min1,int*** max1)//参数还待添加  account记录变量个数
{
    ...
    //ChangeQ(x,account);//更改变量的实际论域值 3.16.yjy
    ...
    while(!delist.empty())//!DelistEmpty(delist,account)) !empty？？？
    {
        ...
        for(int k=0; k!=account; k++)
            if(matrix[k][xj]!=-1)
                HelpRecordC.push(k);//记录所有和xj构成Cij的xi,hacContrain没起到作用 15.3.16.night yjy
        while(!HelpRecordC.empty())
        {
            ...
            {
                //没实际意义额 3.16.yjy
               /* if (!x[xi].IfInLowerLayer(p))//在L中判定假删除？？？？？？？
                {
                    continue;
                }*/
                          ...
                          for(int iter=0;iter<account;iter++)
        {
            Keep_The_Same_Q_L(x,iter);
             ...
             //   GetBackTheDom(TempArry1,x,xj,p);//最后一个参数是值，不是在论域中的标号，WARNNING  3.16.yjy
            }//不是P吧？
        }//while(!HelpRecord)

    }//while(delist)
  //  ChangeQ(x,account);没有作用 3.16.yjy
    return true;
